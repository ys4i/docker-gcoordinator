...
ルートディレクトリには利用者が実行するランチャーだけを配置しています。

- `install-linux.sh`
- `install-macos.sh`
- `install-windows.ps1`
- `start-gcoordinator-macos.command`
- `start-gcoordinator.cmd`

セットアップ、起動、コンテナ内部処理などの実装スクリプトは`scripts/`配下に
まとめています。通常はルートのOS別ランチャーを使用してください。
...
```bash
mkdir -p log
./scripts/run-linux.sh 2>&1 | tee "log/run-linux-$(date +%Y%m%d-%H%M%S).log"
```
...
```bash
bash ./scripts/setup-macos.sh --no-launch
```

`sh scripts/setup-macos.sh`のように`sh`を明示して実行しないでください。
このスクリプトはBash用です。
...
```bash
MACOS_VNC_PORT=5901 MACOS_VNC_PASSWORD=gcoord01 bash ./scripts/run-macos.sh
```
...
```bash
cd ~/Projects/docker-gcoordinator-main && bash ./scripts/setup-macos.sh
```
...
```powershell
.\scripts\setup-windows.ps1 -Mode VcXsrv -SkipInstall
```
...
```powershell
.\scripts\run-windows.ps1 -Mode VcXsrv
```
...
.\scripts\run-windows.ps1 2>&1 | Tee-Object -FilePath "log\run-windows-$(Get-Date -Format yyyyMMdd-HHmmss).log"
```
...
```bash
./scripts/setup-wsl-docker.sh
./scripts/setup-wslg.sh
```
...
```bash
mkdir -p log
./scripts/run-wslg.sh 2>&1 | tee "log/run-wslg-$(date +%Y%m%d-%H%M%S).log"
```
